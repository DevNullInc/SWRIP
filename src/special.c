...
*/
           default:
                 snprintf ( buf , sizeof(buf), "Hmm, a %s." , race_table[victim->race].race_name );
        ...
        snprintf ( buf , sizeof(buf), "%s steps out and the shuttle quickly returns to the academy.\n\r" , victim->name );
        echo_to_room( AT_ACTION , ch->in_room, buf );
    }
...
bool spec_customs_smut( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_SMUT)
	    {
	       ...
	       {
	          snprintf( buf , sizeof(buf), "%s is illegal contraband. I'm going to have to confiscate that.", obj->short_descr );
                  do_say( ch , buf );
...
bool spec_customs_weapons( CHAR_DATA *ch )
{
	...
	for ( obj = victim->last_carrying; obj; obj = obj->prev_content )
	{
	    if (obj->pIndexData->item_type == ITEM_WEAPON)
	    {
	       if ( victim != ch && can_see( ch, victim ) && can_see_obj( ch,obj ) )
	       {
	          snprintf( buf , sizeof(buf), "Weapons are banned from non-military usage. I'm going to have to confiscate %s.", obj->short_descr );
                  do_say( ch , buf );
                  if ( obj->wear_loc != WEAR_NONE )
                    remove_obj( victim, obj->wear_loc, TRUE );
                  separate_obj( obj );
...
bool spec_police( CHAR_DATA *ch )
{
	...
	for ( vip = 0 ; vip < 32 ; vip++ )
          if ( IS_SET ( ch->vip_flags , 1 << vip ) &&  IS_SET( victim->pcdata->wanted_flags , 1 << vip) ) 
          {
              ...
              snprintf( buf , sizeof(buf), "Hey you're wanted on %s!", planet_flags[vip] );
                  do_say( ch , buf );                 
...
bool spec_police_undercover( CHAR_DATA *ch )
{
	...
	for ( vip = 0 ; vip < 32 ; vip++ )
          if ( IS_SET ( ch->vip_flags , 1 << vip ) &&  IS_SET( victim->pcdata->wanted_flags , 1 << vip) ) 
          {
              snprintf( buf , sizeof(buf), "Got you!" );
                  do_say( ch , buf );                 
              REMOVE_BIT( victim->pcdata->wanted_flags , 1 << vip ); 	                   
              multi_hit( ch, victim, TYPE_UNDEFINED );
              return TRUE;         
...
bool spec_guardian( CHAR_DATA *ch )
{
    ...
    {
	snprintf( buf, sizeof(buf), "%s is a %s!  As well as a COWARD!",
		victim->name, crime );
	do_yell( ch, buf );
	return TRUE;
    }
    ...
    if ( victim )
    {
	snprintf( buf, sizeof(buf), "%s is a %s!  PROTECT THE INNOCENT!!",
		victim->name, crime );
	do_shout( ch, buf );
	multi_hit( ch, victim, TYPE_UNDEFINED );
	return TRUE;
