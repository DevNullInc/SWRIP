...
void do_bet(CHAR_DATA *ch, char *argument)
 {
   ...
   else if(num_gladiators > 0 && !in_start_arena)
   {
   ...
   else
      {
         ...
         snprintf(buf, sizeof(buf), "You place %d credits on %s.\r\n", newbet, ch->betted_on->name);
         send_to_char(buf, ch);
         snprintf(buf, sizeof(buf), "%s has placed %d credits on %s.", ch->name, newbet, ch->betted_on->name);
...
void do_arena(CHAR_DATA *ch, char *argument)
{
 ...
 {
  snprintf(buf, sizeof(buf), "Sorry but you must be at least level %d to enter this arena.\r\n", lo_lim);
   send_to_char(buf, ch);
   return;	
 }
 ...
 else
 {
    ...
    do_look(ch, "auto");
  snprintf(buf, sizeof(buf), "%s has joined the blood bath.", ch->name);
    to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
    send_to_char(buf, ch);
...
void do_marena(CHAR_DATA *ch, char *argument)
{
 ...
 else
 {
  ...
  snprintf(buf, sizeof(buf), "%s has been whisked away to the killing fields.\n\r", mob->name);
    send_to_char(buf,ch);
    ...
    send_to_char("You have been taken to the killing fields\r\n",mob);
    do_look(mob, "auto");
  snprintf(buf, sizeof(buf), "%s has joined the blood bath.", mob->name);
    to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
...
void do_chaos(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, sizeof(buf), "LowLim %d HiLim %d Delay %d Length %d\n\r", lo_lim, hi_lim, start_time, game_length);
  send_to_char(buf,ch);
...
void start_arena()
{
 ...
 {   
    ...
    else
    {
      if(time_to_start >1)
      {
         snprintf(buf1, sizeof(buf1), "&WThe Killing Fields are open to levels &R%d &Wthru &R%d\r\n", lo_lim, hi_lim);
         snprintf(buf1, sizeof(buf1), "%s%d &Whours to start\r\n", buf1, time_to_start);
         snprintf(buf2, sizeof(buf2), "The killing fields are open.\n\r");
         snprintf(buf2, sizeof(buf2), "%s&R%d &Whour to start\n\r", buf2, time_to_start);
      }
      else
      {
         snprintf(buf1, sizeof(buf1), "&WThe Killing Fields are open to levels &R%d &Wthru &R%d\r\n", lo_lim, hi_lim);
         snprintf(buf1, sizeof(buf1), "%s1 &Whour to start\r\n", buf1);
         snprintf(buf2, sizeof(buf2), "The killing fields are open.\n\r");
         snprintf(buf2, sizeof(buf2), "%s&R1 &Whour to start\n\r", buf2);
      }
  snprintf(buf1, sizeof(buf1), "%sType &Rarena &Wto enter.\r\n", buf1);
      to_channel(buf1,CHANNEL_ARENA,"&RArena&W",lo_lim);
  snprintf(buf2, sizeof(buf2), "%sPlace your bets!!!\n\r", buf2);
  ...
  if (!(num_gladiators)) {
    ...
    else 
    {
      if(time_to_start >1)
      {
  snprintf(buf1, sizeof(buf1), "The duel will start in %d hours. Place your bets!",
      time_to_start);
      }
      else
      {
  snprintf(buf1, sizeof(buf1), "The duel will start in 1 hour. Place your bets!");
      }
...
void do_game()
{
  ...
  {
     snprintf(buf, sizeof(buf), "With %d hours left in the game there are %d players left.",
       time_left_in_game, num_gladiators);
     to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
  }
  else if(!in_start_arena && time_left_in_game == 1)
  {
  snprintf(buf, sizeof(buf), "With 1 hour left in the game there are %d players left.",
          num_gladiators);
  to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
  }
  ...
  for(g=gladiators;g;g=g->next) {
	...
	i->hit > 0 && g->place == 0) {
      ...
      if(time_left_in_game == 1) {
               snprintf(buf, sizeof(buf), "After 1 hour of battle %s is declared the winner",i->name);
               to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
      } else {
       snprintf(buf, sizeof(buf), "After %d hours of battle %s is declared the winner",
         game_length - time_left_in_game, i->name);
       to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
      }

      snprintf(buf, sizeof(buf), "You have been awarded %d credits for winning the arena\r\n",
       (arena_pot/2));
      i->gold += (arena_pot/2);
      send_to_char(buf, i);
      snprintf(buf2, sizeof(buf2), "%s awarded %d credits for winning arena", i->name,
        (arena_pot/2));
      bug(buf2, 0);
...
void show_jack_pot()
{
  ...
  snprintf(buf1, sizeof(buf1), "%sThe jack pot for this arena is %d credits\r\n", buf1, arena_pot);
  snprintf(buf1, sizeof(buf1), "%s%d credits have been bet on this arena.\r\n", buf1, bet_pot);
  to_channel(buf1,CHANNEL_ARENA,"&RArena&W",5);
...
void silent_end()
{
  ...
  arena_pot = 0;
  bet_pot = 0;
  snprintf(buf, sizeof(buf), "It looks like no one was brave enough to enter the Arena.");
  to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
  reset_bets();
  g = gladiators;
  while(gladiators) {
...
void do_end_game()
{
  ...
  snprintf(buf, sizeof(buf), "After %d hours of battle the Match is a draw", game_length);
     to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
     time_left_in_game = 0;
     ppl_challenged = 0;
     reset_bets();
...
void do_awho(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, sizeof(buf), "&W  Players in the &BRise in Power&W Arena\r\n");
  snprintf(buf, sizeof(buf), "%s-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-", buf);
  snprintf(buf, sizeof(buf), "%s&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-\r\n", buf);
  snprintf(buf, sizeof(buf), "%sGame Length = &R%-3d   &WTime To Start &R%-3d\r\n", buf, game_length, time_to_start);
  snprintf(buf, sizeof(buf), "%s&WLevel Limits &R%d &Wto &R%d\r\n", buf, lo_lim, hi_lim);
  snprintf(buf, sizeof(buf), "%s         &WJackpot = &R%d\r\n", buf, arena_pot);
  snprintf(buf, sizeof(buf), "%s&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B", buf);
  snprintf(buf, sizeof(buf), "%s-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B-&W-&B\r\n", buf);
  send_to_char(buf, ch);

 
  for(g = gladiators; g; g=g->next)
    if (g->ch && g->place == 0) {
  snprintf(buf2, sizeof(buf2), "&W%s\n\r", g->ch->name);
...
void do_ahall(CHAR_DATA *ch, char *argument)
{
  ...
  if (!fame_list)
  {
    ...
    snprintf(buf2, sizeof(buf2), "&B|---------------------------------------|\r\n");
    strncat(buf2, "| &WPast Winners of The Rise in Power Arena&B  |\r\n", sizeof(buf2) - strlen(buf2) - 1);
    strncat(buf2, "|---------------------------------------|\r\n\r\n", sizeof(buf2) - strlen(buf2) - 1);
    ...
    for (fame_node = fame_list; fame_node; fame_node = fame_node->next)
    {
      ...
      strcpy(site, "Unknown");
    snprintf(buf, sizeof(buf), format2, fame_node->name, site, fame_node->award);
    send_to_char(buf, ch);
    }
    return;
 }
...
void find_bet_winners(CHAR_DATA *winner)
{
  ...
  for (d = first_descriptor; d; d = d->next)
    if (!d->connected)
    {
       ...
       {
          snprintf(buf1, sizeof(buf1), "You have won %d credits on your bet.\r\n",(GET_BET_AMT(wch))*2);
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
 ...
 snprintf(buf, sizeof(buf), "&R%s &Whas challenged you to a duel!\n\r",ch->name);
 send_to_char(buf,victim);
 send_to_char("&WPlease either accept or decline the challenge.\n\r\n\r",victim);
 snprintf(buf, sizeof(buf), "%s has challenged %s to a duel!!\n\r",ch->name,victim->name);
 to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
...
void do_accept(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, sizeof(buf), "%s has accepted %ss challenge!\n\r",ch->name,dch->name);
  to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
    ch->challenged = NULL;
    ch->retran = ch->in_room->vnum;
    char_from_room(ch);
...
void do_decline(CHAR_DATA *ch, char *argument)
{
 ...
 if (ch->challenged)
 {
  snprintf(buf, sizeof(buf), "%s has DECLINED %ss challenge! WHAT A WUSS!!!\n\r",ch->name,ch->challenged->name);
  to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
  ch->challenged=NULL;
  return;
 }
...
void remove_from_arena(CHAR_DATA *ch) 
{
  ...
  for(g = gladiators;g;g=g->next) {
    if (g->ch == ch) {
      ...
      switch (g->place) {
        case 1: snprintf(buf, sizeof(buf), "%s is out of the fight in 1st place.",ch->name); break;
        case 2: snprintf(buf, sizeof(buf), "%s is out of the fight in 2nd place.",ch->name); break;
        case 3: snprintf(buf, sizeof(buf), "%s is out of the fight in 3rd place.",ch->name); break;
        default:snprintf(buf, sizeof(buf), "%s is out of the fight in %dth place.",ch->name,g->place); break;
      }
      to_channel(buf,CHANNEL_ARENA,"&RArena&W",5);
      num_gladiators--;
    }
